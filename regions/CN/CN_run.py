...
def main(output_path: Path, json_output_path: Path):
    ...
    addressable_url = update_urls_cn.get_addressable_catalog_url(
        server_url_json, json_output_path
    )
    ...
    try:
        server_dict = json.loads(server_url_json)
    except Exception as e:
        notice(f"解析 server_url.json 失败: {e}")
        return

    table_version = server_dict.get("TableVersion")
    if not table_version:
        notice("server_url.json 中没有 TableVersion 字段")
        return

    table_manifest_url = f"{addressable_url}/Manifest/TableBundles/{table_version}/TableManifest"
    ...
    lines[4] = f"BA_SERVER_URL_CN={table_manifest_url}\n"
