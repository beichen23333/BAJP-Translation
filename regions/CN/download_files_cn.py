...
def download_excel_files(env_file: Path, output_dir: Path):
    ...
    table_manifest_url = env_vars.get("BA_SERVER_URL_CN")
    if not table_manifest_url:
        raise KeyError("环境变量 BA_SERVER_URL_CN 不存在")

    addressable_catalog_url = env_vars.get("ADDRESSABLE_CATALOG_URL_CN")
    if not addressable_catalog_url:
        raise KeyError("环境变量 ADDRESSABLE_CATALOG_URL_CN 不存在")
    ...
    excel_db_download_url  = f"{addressable_catalog_url}/pool/TableBundles/{crc_prefix}/{crc_value}"
    excel_zip_download_url = f"{addressable_catalog_url}/pool/TableBundles/{crc_prefix2}/{crc_value2}"
